...
EntityPlayer = ig.Box2DEntity.extend({
	...
	animSheet: new ig.AnimationSheet( 'media/images/player.png', 30, 30 ),	
	...
	shoot: function(weapon) {
		...
		if(weapon == 'kick') {
			ig.game.spawnEntity(EntityShockwave, x, y, {followEntity: this});
			return;
		}

		if(weapon == 'snare') {
			ig.game.spawnEntity(EntityScatterShot, x, y);
			return;
		}

		if(weapon == 'hh_closed') {
			ig.game.spawnEntity(EntityShapeBullet, x, y, {shape: 'chevron'});
			return;
		}

		ig.game.spawnEntity(EntityProjectile, x, y, {frame: this.frames[weapon]});
	},
